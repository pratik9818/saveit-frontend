...
export default function FragmentChild({
  ...
  const [selectedFragmentValue, setSelectedFragment] =
    useRecoilState(selectedFragment);
  let fragmentDimension;
  let fragmentMargin = "ml-auto";
  if (clientwidth > breakFragmentPixel)
    fragmentDimension = fragmentDimensionAbove700px;
  else {
  ...
  const getFragmentsDimension = fragmentDimension.find((element) => {
  ...
  function multiFragmentSelect() {
    if (selectedFragmentValue.includes(fragmentdetails)) {
      setSelectedFragment(
        selectedFragmentValue.filter(
          (fragment) => fragment.fragment_id !== fragmentdetails.fragment_id
        )
      );
